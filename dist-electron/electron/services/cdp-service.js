...
// Injection scripts
import { FIBER_EXTRACTOR_SCRIPT, TRIGGER_EVENT_SCRIPT, GET_STATE_SCRIPT, WATCH_STATE_SCRIPT } from "./react-fiber-extractor.js";
let client = null;
let connectedUrl = "";
...
// ── Connection ──────────────────────────────────────────────
export async function connect(host, port, targetUrl) {
    try {
        status = "connecting";
        // List available targets
        const targets = await CDP.List({ host, port });
        // Find the right tab
        let target = targets.find((t) => t.type === "page" && (!targetUrl || t.url.includes(targetUrl)));
        if (!target) {
        ...
        client = await CDP({ host, port, target: target });
...
`;
async function probeEventFlows(entities, edges) {
    if (!client)
        return;
        ...
        for (const handler of el.eventHandlers) {
            ...
            try {
                ...
                if (result.result.value && Array.isArray(result.result.value)) {
                    const changedStateIds = result.result.value;
...
// ── Event Triggering ────────────────────────────────────────
export async function triggerEventLive(elementEntityId, eventName, inputValue) {
    if (!client || status !== "connected") {
...
// ── State Reading ───────────────────────────────────────────
export async function getStateLive(componentEntityId) {
    if (!client || status !== "connected") {
        throw new Error("Not connected");
    }
