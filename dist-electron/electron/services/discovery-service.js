...
export async function discoverTargets(host = "localhost") {
    const results = [];
    // Check CDP ports
    ...
    const deduped = results.filter(r => {
        if (r.type === "devserver" && cdpUrls.has(r.url))
            return false;
        return true;
    });
    return deduped;
}
async function fetchCdpTabs(host, port) {
...
async function probeDevServer(host, port) {
...
export async function launchChromeWithDebugging(targetUrl, cdpPort = 9222) {
    const chromePaths = [
        "google-chrome",
    ...
    for (let i = 0; i < 16; i++) {
        ...
        if (tabs && tabs.some((t) => t.type === "page")) {
            return { success: true };
        }
    }
...
function detectFramework(html) {
    ...
    if (html.includes("ng-version") || html.includes("ng-app"))
        return "Angular";
    if (html.includes("/src/main.tsx") || html.includes("/src/main.ts") || html.includes("/@vite"))
        return "Vite";
    if (html.includes("bundle.js") || html.includes("/static/js/"))
        return "React (CRA)";
