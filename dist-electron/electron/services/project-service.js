...
function extractEntities(sourceFile, relativePath) {
    const entities = [];
    function visit(node) {
        ...
        if (ts.isClassDeclaration(node) && node.name && hasExportModifier(node)) {
            ...
            for (const member of node.members) {
                if (ts.isMethodDeclaration(member) &&
                    ...
                    if (classEntity)
                        classEntity.dependencies.push(methodId);
                }
            }
        }
    ...
    for (const comp of componentEntities) {
        ...
        const setterToStateId = new Map(); // setterName → state entity ID
        ...
        const localHandlers = funcBody ? collectLocalHandlers(funcBody, sourceFile) : new Map();
        for (const el of uiElements) {
            ...
            for (const [eventName, handlerExpr] of Object.entries(el.eventHandlers)) {
                const info = traceEventHandler(eventName, handlerExpr, localHandlers, setterToStateId, sourceFile);
                if (info)
...
function resolveLocalImport(fromDir, specifier, projectPath, allFiles) {
...
function buildEdges(entities, fileImports, sourceFiles) {
    const edges = [];
    const seen = new Set();
    ...
    const fileImportMap = new Map(); // filePath -> (importedName -> entityId)
    for (const [fromFile, importList] of fileImports) {
        ...
        for (const imp of importList) {
            ...
            for (const importedName of imp.namedImports) {
                ...
                if (targetEntity)
                    nameToEntityId.set(importedName, targetEntity.id);
            }
            if (imp.hasDefault) {
                const defaultEntity = toEntities.find((e) => e.exportName === "default");
                if (defaultEntity)
                    nameToEntityId.set("default", defaultEntity.id);
            }
        }
...
async function getOrLoadModule(modulePath) {
    ...
    for (const [modPath, mod] of moduleCache) {
        ...
        for (const [key, value] of Object.entries(mod)) {
            if (typeof value === "function")
                continue; // skip functions/classes
            try {
                // Ensure serializable
        ...
        if (Object.keys(vars).length > 0)
            moduleVariables[relPath] = vars;
    }
    const instances = Array.from(liveInstances.entries()).map(([id, info]) => {
        const props = safeSerialize(info.instance);
...
export async function setVariable(filePath, varName, valueJson) {
    if (!lastScan)
        throw new Error("No project scanned");
    const modulePath = path.resolve(lastScan.projectPath, filePath);
    const mod = await getOrLoadModule(modulePath);
    const parsed = JSON.parse(valueJson);
    mod[varName] = parsed;
}
export async function runEntity(request) {
    ...
    try {
        ...
        if (entity.kind === "class-method" && entity.name.includes(".")) {
            ...
            if (request.instanceId && liveInstances.has(request.instanceId)) {
                ...
                const method = info.instance[methodName];
                if (typeof method !== "function")
                    throw new Error(`Method "${methodName}" not found on instance`);
                output = await method.apply(info.instance, args);
                ...
                if (typeof method === "function") {
                    output = await method.call(instance);
                }
                else {
                    output = instance;
                }
            ...
            if (output && typeof output === "object" && output.constructor && output.constructor.name !== "Object" && output.constructor.name !== "Array") {
                ...
                liveInstances.set(instanceId, { className: output.constructor.name, instance: output, filePath: entity.filePath });
            }
        }
 ...
 */
function resolveArgs(inputJson) {
    ...
    while (i < processed.length) {
        if (processed[i] === '"') {
            let j = i + 1;
            while (j < processed.length && !(processed[j] === '"' && processed[j - 1] !== '\\'))
                j++;
...
async function loadModule(modulePath) {
    if (!tsxRegistered) {
        try {
            const tsxApi = _require("tsx/cjs/api");
            tsxApi.register();
        }
        catch (e) {
...
function extractHooksFromSource(sourceFile, componentName) {
...
function buildHookInfo(kind, hookName, decl, callExpr, sourceFile, counters) {
    ...
    const displayParts = [varName];
    if (typeHint)
        displayParts.push(`: ${typeHint}`);
    else if (initialValue)
        displayParts.push(` = ${initialValue}`);
    const params = [];
    if (kind === "state") {
        params.push({ name: varName, type: typeHint ?? "unknown" });
        if (setterName)
            params.push({ name: setterName, type: `(value) => void` });
    }
    else if (kind === "ref") {
...
function extractEffectDeps(callExpr, sourceFile) {
...
function findExportedFunctionBody(sourceFile, name) {
    for (const stmt of sourceFile.statements) {
...
function extractExportedVariables(sourceFile, relativePath, entities) {
    for (const stmt of sourceFile.statements) {
...
function collectLocalHandlers(funcNode, sourceFile) {
...
function traceEventHandler(eventName, handlerExpr, localHandlers, setterToStateId, sourceFile) {
    ...
    function walkForSetters(node, phase) {
...
function classifySetterPattern(setterName, callExpr, sourceFile) {
...
function findEntityNode(sourceFile, entity) {
    for (const stmt of sourceFile.statements) {
        // export function foo(...)
...
function hasPublicVisibility(member) {
...
function buildFunctionEntity(filePath, exportName, params, returnType) {
    return {
...
function extractUiElementsFromSource(sourceFile, name) {
    for (const stmt of sourceFile.statements) {
        if (ts.isFunctionDeclaration(stmt) && stmt.name?.text === name) {
    ...
    function walk(n) {
        ...
        if (isOpening || isSelfClosing) {
            ...
            for (const evtName of eventAttrNames) {
                if (attrs[evtName])
                    eventHandlers[evtName] = attrs[evtName];
            }
...
function getJsxTextContent(element) {
    const texts = [];
...
function buildElementLabel(tag, attrs, textContent, counters) {
    ...
    if (textContent)
        return textContent.replace(/[^a-zA-Z0-9 ]/g, "").trim().slice(0, 25);
    if (attrs["onClick"])
        return `onClick:${attrs["onClick"]}`;
    if (attrs["onChange"])
        return `onChange:${attrs["onChange"]}`;
    if (attrs["type"])
        return `type:${attrs["type"]}`;
    if (attrs["placeholder"])
        return attrs["placeholder"].slice(0, 25);
    if (attrs["href"])
        return `href:${attrs["href"].slice(0, 25)}`;
    // Fallback: indexed
...
function buildDisplayName(tag, attrs, textContent) {
    if (textContent)
        return `<${tag}> "${textContent}"`;
    if (attrs["aria-label"])
        return `<${tag}> "${attrs["aria-label"]}"`;
    if (attrs["placeholder"])
        return `<${tag}> [${attrs["placeholder"]}]`;
    if (attrs["type"])
    ...
    if (attrs["onClick"])
        return `<${tag}> onClick=${attrs["onClick"]}`;
    return `<${tag}>`;
}
/** Map HTML tag names to their semantic actions (PRD section 7.5) */
function getUiElementActions(tag) {
    const actionMap = {
...
function buildComponentEntity(filePath, exportName, params) {
    return {
        id: `${filePath}#${exportName}`,
        kind: "component",
        name: exportName,
...
async function collectFiles(projectPath) {
    const result = [];
